...
import type { JournalStory } from './page';
...
// ─── Journal Card ────────────────────────────────────────────────────────
function JournalCard({ story, index }: { story: JournalStory; index: number }) {
    ...
    >
      ...
      >
        <div className="relative h-52 overflow-hidden">
          {story.imageUrl && !imgError ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-amber-50 to-orange-50">
              <BookOpen className="h-12 w-12 text-foreground/10" />
            </div>
          )}
        ...
        <div className="flex flex-1 flex-col p-5">
          ...
          <h3 className="text-lg font-semibold text-[#111827] transition-colors group-hover:text-teal-600 line-clamp-2">
            {story.title}
          </h3>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-amber-50 text-amber-600 transition-colors group-hover:bg-amber-100">
...
// ─── Featured Story Card (Horizontal / Hero Highlight) ──────────────────
function FeaturedStoryCard({ story }: { story: JournalStory }) {
  const [imgError, setImgError] = useState(false);
    ...
    >
      ...
      >
        ...
        <div className="flex flex-1 flex-col justify-center p-4">
          ...
          <h3 className="mt-2 text-sm font-semibold text-[#111827] line-clamp-2 group-hover:text-teal-600 transition-colors">
            {story.title}
          </h3>
...
export function JournalPageClient({ stories }: Props) {
  ...
  const POPULAR_SUGGESTIONS = [
    { label: 'Rajasthan Travel Guide', icon: MapPin },
    { label: 'Himalayan Retreats', icon: Compass },
    { label: 'Kerala Backwaters', icon: TrendingUp },
    { label: 'Luxury Wellness', icon: Sparkles },
    { label: 'Heritage Hotels', icon: BookOpen },
    { label: 'Indian Cuisine', icon: Compass },
  ];
      ...
      >
        ...
        >
          <div
            ...
            style={{
              backgroundImage:
                'url(https://images.unsplash.com/photo-1506905925346-21bda4d32df4?auto=format&fit=crop&w=1920&q=85)',
            }}
          />
          {/* Dark Overlay */}
        ...
        >
          <div className="mx-auto max-w-4xl text-center">
            ...
            >
              Curated travel stories, destination deep dives, and insider guides
              from India's most discerning explorers and luxury travel connoisseurs.
            </motion.p>
            ...
            >
              {[
                ...
                { label: 'Authors', value: new Set(stories.map((s) => s.author)).size },
        ...
        {filteredStories.length === 0 ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {filteredStories.map((story, index) => (
              <JournalCard key={story.id} story={story} index={index} />
            ))}
          </div>
        )}
      </section>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-amber-50 via-orange-50/40 to-teal-50/40 px-8 py-14 text-center md:px-16 md:py-20">
          ...
          <div className="relative z-10 mx-auto max-w-2xl">
            ...
            <p className="mx-auto mt-4 max-w-lg text-sm text-muted-foreground">
              We're always looking for fresh perspectives on luxury travel in
              India. Share your experiences, tips, and hidden gems with our
              community of discerning travellers.
            </p>
